...
class YoloV2Detector(darknetDetector.DarknetAbstract):
    ...
    def transform(self, dataset_path, classes_path, output_path):
        ...
        classes_name = open(os.path.join(output_path,"classes.name"), "w")
        for line in f:
    ...
    def organize(self, dataset_path, output_path, train_percentage):
        ...
        shutil.copy(os.path.join(dataset_path[:dataset_path.rfind(os.sep)], "classes.name"), os.path.join(output_path, dataset_name))
    ...
    def createModel(self, dataset_path):
        ...
        file = open(os.path.join(dataset_path, "classes.name"))
        classes = []
        for line in file:
            classes.append(line)
    ...
    def train(self, dataset_path):
        ...
        print("./darknet/darknet detector train " + os.path.abspath(dataset_path+ os.sep + data) + " " +
              os.path.abspath(dataset_path+ os.sep + confi) + " objectDetectors/YOLOObjectDetector/darknet53.conv.74")
    ...
    def evaluate(self, dataset_path):
        ...
        print(
            "./darknet/darknet detector map " + os.path.abspath(dataset_path + os.sep + data) + " " + os.path.abspath(
                dataset_path + os.sep + confi) + " objectDetectors/YOLOObjectDetector/darknet53.conv.74")
